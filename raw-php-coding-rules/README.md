...
### Prefer elseif over else if [colon syntax]
- else if is not compatible with the colon syntax for if|elseif blocks. For this reason, use elseif for conditionals.

```php 
    if(something==1):
        <sgow something related to 1>
    elseif(something==2):
        <sgow something related to 2>
     endif;
```
 ...
 * Image uploading /Email sending  should be handled via background jobs/queue.
 * Auth::check() should be done via middleware.
 * Core PHP/HTTP methods such as $_POST etc should not be used in frameworks
 * Constant should be prefered over define
 * Redirection should be made via .htaccess (over php)
 * PHP supports one error control operator: the at sign (@). When prepended to an expression in PHP, any error messages that might be generated by that expression will be ignored.It sould be avoided
 * "print” statement is slower than “echo” statement.
 * Prefer Single Quotes around Array Indexes php
	https://stackoverflow.com/questions/29446901/is-it-mandatory-to-use-quotessingle-quotes-double-quotes-around-an-index-whi#answer-29448708
